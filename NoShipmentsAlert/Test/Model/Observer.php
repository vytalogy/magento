 ...
 * @author    Saad Yusuf
 */
class NoShipmentsAlert_Test_Model_Observer extends EcomDev_PHPUnit_Test_Case
{
    /**
     * const string
     *
     ...
     *
     * @var NoShipmentsAlert_Model_Observer
     */
    ...
    {
        $this->_model   = Mage::getModel('noshipmentsalert/observer');
     ...
     *
     * Test the shipment alert does not get added to the collection
     *
     * @return null
     *
     * @test
    ...
    public function testNoAlertNew()
    {
        ...
        $this->assertInstanceOf(
            'NoShipmentsAlert_Model_Observer', $this->_model,
            'Model is an instance of NoShipmentsAlert_Model_Observer'
        );

        // update the fixture created_at field
        // so the shipment time is more recent than the alert time
        $shipment = Mage::getModel('sales/order_shipment')->load(1);
        $newTime  = Mage::app()->getLocale()->storeTimeStamp($shipment->getStoreId());
        $newTime  = date("Y-m-d H:m:s", $newTime);
        $shipment->setCreatedAt($newTime);
        $shipment->save();
        ...
        $this->assertEquals(
            0, count($emails_sent),
            'Check no email have been sent - order to new'
        );
    }
    ...
    public function testSendAlertOld()
    {
        ...
        $this->assertEquals(
            1, count($emails_sent),
            'Check alert email have been sent - shipment is old'
        );
     ...
     *
     * Test the shipment alert gets added to the collection as it match's the alert time
     *
     * @return null
     *
     * @test
    ...
    public function testSendAlert()
    {
        ...
        // get the alert time
        $alertTime = Mage::getStoreConfig(NoShipmentsAlert_Helper_Data::PATH_ALERT_TIME);
        ...
        if ($alertTime == 1) {
            ...
            $newTime = strtotime('-'.$alertTime.' hours', $newTime);
        }
        ...
        $this->assertEquals(
            1, count($emails_sent),
            'Check alert email have been sent - shipment time = alert time'
        );
